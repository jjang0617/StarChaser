...
### 2. 카카오 로그인 & 회원가입 통합 (OAuth 2.0)
* **네이티브 모바일 최적화**: 모바일 웹뷰(WebView) 환경에서 네이티브 앱 전환 시 발생하는 signature mismatch 오류를 없애기 위해 웹 인터페이스를 정밀 커스터마이징했습니다.
* **정돈된 UI**: 웹뷰 내의 불필요한 가이드 텍스트("계정 정보 입력으로도...", "또는") 및 미지원 카카오톡 직접 연결 버튼을 DOM 주입을 통해 완전히 숨겼습니다. 로그인 폼 마진을 축소하고 인풋 필드와 로그인 버튼 크기를 모바일 터치 규격($48\text{px}$)으로 확대하여 접근성을 개선했습니다.
* **계정 자동 식별 및 보안**: 카카오 계정 로그인 시 비밀번호 변경/찾기 버튼을 자동으로 숨겨 OAuth 계정의 특징을 유지하며, 회원 탈퇴 시에도 비밀번호 확인 단계를 거치지 않고 안전하게 원클릭으로 탈퇴가 이루어지도록 설계했습니다.

### 3. 카카오맵 기반 별 관측 명소 탐색 (Explore Map)
* **클러스터링 및 맞춤 마커**: 전국 별 관측 명소의 실시간 Star-Index 정보를 카카오맵 웹뷰에 핀 형태로 표시합니다.
* **명소 상세 편의 시설 정보**: 각 명소 카드 하단에 고도 정보(`▲ 390m` 등)와 함께, 해당 명소의 **주차 가능 여부(`주차 O`/`주차 X`)**와 **화장실 구비 여부(`화장실 O`/`화장실 X`)**를 고유 브랜드 컬러(연한 파란색 / 연한 초록색) 뱃지로 가독성 있게 상시 노출합니다. (야간 모드 적용 시에는 눈부심 방지를 위해 붉은색 뱃지로 자동 전환됩니다.)
...
### 5. 가상 스카이 뷰어 (Virtual Sky Viewer)
* **실시간 성도 렌더링**: `expo-gl` WebGL Canvas 기술을 활용하여 사용자가 서 있는 위치와 시간에 맞는 별자리와 실시간 가상 밤하늘 지도를 스마트폰 화면에 3D로 시뮬레이션 렌더링합니다.
