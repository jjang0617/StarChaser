...
주의: StarChaser의 lunPhase(0~1)는 현재 달 주기 진행률에 가까운 값이며, Stellarium의 Phase(%)는 조도(illumination) 비율이다.
두 값은 동일 지표가 아니므로 직접 수치 비교 대신 지표 변환 또는 정의 통일이 필요하다.
### B-6. 결과 해석 (쉽게 보기)
...
## 지표 정의 차이(B-6 위 주의 참조)
- StarChaser lunPhase와 Stellarium Phase는 동일 지표가 아님
- 직접 수치 비교 시 오해 가능
- 팀에서 기준 지표(진행률/조도) 통일 필요
